def resistor_label(colors):
    global ohms, tolerance
    colors_list = ['black', 'brown', 'red', 'orange', 'yellow', 'green', 'blue', 'violet', 'grey', 'white']
    tolerance_list = {'grey': '0.05%', 'violet': '0.1%', 'blue': '0.25%', 'green': '0.5%', 'brown': '1%', 'red': '2%',
                      'gold': '5%', 'silver': '10%'}
    if len(colors) == 1:
        ...
        ohms = ((100 * colors_list.index(colors[0])) + (10 * colors_list.index(colors[1])) + colors_list.index(
            colors[2])) * 10 ** colors_list.index(colors[3])
